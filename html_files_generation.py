...
def html_builder(id: int, all_data_dates_and_marks: dict, data_work_time_all_employees: dict, all_data_per_month_employees: dict, total_salary_employees: dict):
    """Функция создает html файл, путем вызвова функций, которые формируют необходимые структуры для построения, а так же функцию 
    неопосредственного построения html файла.Принимает id работника, всю структуру данных, в которой есть данные о всех пользователях за месяц, 
    их отметках прихода и ухода.Кроме того, функция принимает структуру данных,которая содержит информацию об отработанных часах всех сотрудников. 
    Кроме того, структуру данных, содержащую информацию о зарплатах всех сотрудников(при условии ввода корректного секретного ключа). Функция 
    ...
    total_data_for_employee_month = build_data_total_for_month_for_employee(
        id, all_data_per_month_employees, total_salary_employees)
    ...
    data_employee = (
        name_html_file, all_data_per_month_for_marks_employee_for_html, total_data_for_employee_month)
...
def build_data_days_per_month_for_month(id: int, all_data_dates_and_marks: dict, data_work_time_all_employees: dict):
    """Функция создает структуру данных, содержащую информацию о дате, отметки прихода и отметки ухода сотрудника. 
    Функция принимает id сотрудника,структуру данных, которая содержит информацию о датах и отметках всех пользователей. 
    ...
    for date in list_date:
        if id in list_id and id in all_data_dates_and_marks[date]:
            ...
            cell_data_dict['time_end'] = all_data_dates_and_marks[date][id][0].time(
            ).isoformat(timespec='auto')
            cell_data_dict['delta_time'] = str(
                data_work_time_all_employees[date][id][1])
            cell_data_dict['tag_overtime'] = data_work_time_all_employees[date][id][2]
            cell_data_dict['overtime'] = str(
                data_work_time_all_employees[date][id][0])
            ...
            all_data_per_month_for_marks_employee_for_html.append(
                cell_data_dict)
...
def build_data_total_for_month_for_employee(id: int, all_data_per_month_employees: dict, total_salary_employees: dict):
    """Функция создает структуру данных, содержащих суммарную информацию за месяц для одного работника. Функция принимает id  работника,
    ...
    сотрудников(при условии ввода корректного секретного ключа).Функция возвращает словарь, содержаший всю суммарную информацию для конктреного 
    работника,который нужен для построения html файла
    """
    ...
    total_data_for_employee_month['overtime_weekend'] = str_time_overwork(
        data_per_month_employee[1][1])
...
def gen_html_file(data_employee: tuple):
    ...
    Функция принимает кортеж, содержаший имя файла, данные за каждый день для отдельного пользователя в 
    виде словаря и суммарнные данные за месяц, в виде словаря.  
    """
...
def gen_block_topic_table(type_table: int):
    ...
    В любом другом случае - генерируется шапка таблицы сводных данных за месяц. Возвращает список со строками html-файла
    """
    ...
    if type_table == 1:
        ...
        topicsList = ('Фамилия', 'Дата', 'Отметка входа',
                      'Отметка выхода', 'Общее время работы', 'Переработка')
    else:
        topicsList = ('Фамилия', 'Отработано будних дней', 'Переработка в будние дни', 'Рабочих выходных',
                      'Переработка в выходные дни', 'Количество дней отпуска', 'Оклад', 'Молоко', 'Зарплата')
    lines_block = []
...
def gen_block_data_day(data_day: dict):
    ...
    line_2 = 8*space + td_tag_start + \
        data_day['family'] + td_tag_end + new_line
    line_3 = 8*space + td_tag_start + data_day['date'] + td_tag_end + new_line
    if data_day['tag_day'] == 'work':
        line_4 = 8*space + td_tag_start + \
            data_day['time_begin'] + td_tag_end + new_line
        line_5 = 8*space + td_tag_start + \
            data_day['time_end'] + td_tag_end + new_line
        line_6 = 8*space + td_tag_start + \
            data_day['delta_time'] + td_tag_end + new_line
        line_7 = 8*space + td_tag_start + \
            data_day['tag_overtime'] + td_tag_end + new_line
        line_8 = 8*space + td_tag_start + \
            data_day['overtime'] + td_tag_end + new_line
        ...
        line_4 = 8*space + td_tag_merge_cell + \
            data_day['tag_day'] + td_tag_end + new_line
...
def gen_block_data_month_employee(data_month_dict: dict):
    """Функция генерирует блок строк описывающий месяц работы сотрудника. Фамилия, Отработано будних денй, Переработка, 
    Отработанные выходные дни, Переработка в выходные дни,Количесто дней отпуска, Зарплата. Функция принимает словарь с 
    данными за месяц для работника.Возвращет кортеж с строками блока
    """
    ...
    tr_tag_end = '</tr>'
    line_1 = 6*space + tr_tag_start + new_line
    line_2 = 8*space + th_tag_family_start + \
        data_month_dict['family'] + th_tag_end + new_line
    line_3 = 8*space + th_tag_start + \
        str(data_month_dict['all_work_weekdays']) + th_tag_end + new_line
    line_4 = 8*space + th_tag_start + \
        data_month_dict['weekdays_overtime'] + th_tag_end + new_line
    line_5 = 8*space + th_tag_start + \
        str(data_month_dict['work_weekend']) + th_tag_end + new_line
    line_6 = 8*space + th_tag_start + \
        data_month_dict['overtime_weekend'] + th_tag_end + new_line
    line_7 = 8*space + th_tag_start + \
        str(data_month_dict['vacation']) + th_tag_end + new_line
    line_8 = 8*space + th_tag_start + \
        str(data_month_dict['salary']) + th_tag_end + new_line
    line_9 = 8*space + th_tag_start + \
        str(data_month_dict['milk']) + th_tag_end + new_line
    line_10 = 8*space + th_tag_start + \
        str(data_month_dict['salary_whith_milk']) + th_tag_end + new_line
