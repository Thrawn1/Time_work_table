...
def main():
    ...
    for id in employees_who_worked_for_month:
        data_id = build_data_total_for_month_for_employee(id=id,all_data_per_month_employees=work_time_employees_restructuring[0],total_salary_employees=total_salary)
        print('--------------------------------------------------------------------------------------------------------------------------------------------')
        print('\nФамилия работника:  ',data_id['family'])
        print('\n\n\t\t ВСЕГО ЗА МЕСЯЦ:')
        print('\n\n\t')
        print('\n\t\tБудние рабочие дни за месяц:', data_id['all_work_weekdays'])
        print('\t\t\t--------------------------')
        print('\n\t\tПереработки за будние рабочие дни в месяце:', data_id['weekdays_overtime'])
        print('\t\t\t--------------------------')
        print('\n\t\tРабочие выходные дни за месяц:', data_id['work_weekend'])
        print('\t\t\t--------------------------')
        print('\n\t\tПереработки за рабочие выходные дни в месяце:', data_id['overtime_weekend'])
        print('\t\t\t--------------------------')
        print('\n\t\tКоличество дней отпуска:', data_id['vacation'])
        print('\t\t\t--------------------------')
        print('\n\t\tЗарплата(учитвая молоко, но без премий):', data_id['salary_whith_milk'])
        print('\t\t\t--------------------------')
    try:
