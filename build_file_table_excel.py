...
def build_file_excel(time_table: dict, work_time_employees: dict, working_hours_of_workers_sum_of_all_data: dict, total_salary_id: dict):
        ...
        for id in time_table[date]:
            if id != 8 and id != 7:
                ...
                if time_table[date][id][2] == 'work' or time_table[date][id][2] == 'weekend':
                    ws.cell(column=3, row=count,
                            value=time_table[date][id][1].time())
                    ws.cell(count, 3).border = Border(left, right, top, bottom)
                    ws.cell(column=4, row=count,
                            value=time_table[date][id][0].time())
                    ws.cell(count, 4).border = Border(left, right, top, bottom)
                    ws.cell(column=5, row=count,
                            value=work_time_employees[date][id][1])
                    ws.cell(count, 5).border = Border(left, right, top, bottom)
                    ws.cell(column=7, row=count,
                            value=work_time_employees[date][id][0])
                    ws.cell(count, 7).border = Border(left, right, top, bottom)
                    if work_time_employees[date][id][2] == 'недоработка':
                        ws.cell(column=6, row=count,
                                value=work_time_employees[date][id][2])
                        ws.cell(count, 6).border = Border(
                    ...
                    # Сначала границы, потом объединение. Иначе граница будет не верной
                    ws.cell(count, 3).border = Border(left, right, top, bottom)
                    ws.merge_cells(start_row=count, start_column=3,
                                   end_row=count, end_column=7)
                    ws.cell(column=3, row=count, value='Отпуск')
                    ws.cell(column=3, row=count).alignment = Alignment(
                        horizontal='center')
    ...
    topicsList = ['Фамилия', 'Отработано будних день', 'Переработка', 'Рабочих выходных',
                  'Переработка выходных', 'Количество дней дней отпуска', 'Оклад', 'Молоко', 'Зарплата']
    topicCounter = 8
    for topic in topicsList:
    ...
    for id in working_hours_of_workers_sum_of_all_data.keys():
        ...
        ws.cell(column=11, row=count).alignment = Alignment(
            horizontal='center')
        overwork_hours_weekend = number_holiday_day = working_hours_of_workers_sum_of_all_data[
            id][1][1]
        ws.cell(column=12, row=count, value=overwork_hours_weekend)
        ws.cell(count, 12).border = Border(left, right, top, bottom)
        ws.cell(column=13, row=count,
                value=working_hours_of_workers_sum_of_all_data[id][2])
        ws.cell(count, 13).border = Border(left, right, top, bottom)
        ws.cell(column=13, row=count).alignment = Alignment(
            horizontal='center')
    ...
    name_file_excel = month_name_for_print(
        number_month) + '_' + str_year + '.xlsx'
