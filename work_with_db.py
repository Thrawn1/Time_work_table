...
"""
    name_file_with_extension = name_file + '.toml'
    with open(name_file_with_extension, 'w', encoding='utf-8') as file:
...
def create_db(name_db:str) -> None:
    ...
    conn.commit()
    conn.close()
def insert_data_to_db(cursor:Cursor, data:dict, table:str) -> None:
    if table == 'roles':
        ...
        cursor.execute("""INSERT INTO line (id_user, date, time)
                           ...
                           data['time']))
def update_data_to_db(cursor:Cursor, data:dict, table:str) -> None:
    if table == 'roles':
