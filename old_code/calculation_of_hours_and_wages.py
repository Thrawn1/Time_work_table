    ...
    for cell_date in time_table:
        ...
        for cell_id in time_table[cell_date]:
            if cell_id in list_employee[1]:
                hours_worked = time_table[cell_date][cell_id][0] - \
                    time_table[cell_date][cell_id][1]
...
def calculation_of_exceeding_working_hours_per_month(work_time_employees: dict):
    """Функция для подсчета суммарно всех часов переработки в месяц, по каждому сотруднику. Функция принимает словарь,
    ...
    for cell_date in work_time_employees:
        for cell_id in work_time_employees[cell_date]:
            cell_time = (work_time_employees[cell_date][cell_id][0],
                         work_time_employees[cell_date][cell_id][2], cell_date)
    ...
    for id in work_time_employees_restructuring.keys():
        ...
        cell_work_data = ((total_work_days, overwork), (total_holiday_days,
                          overwork_holiday), total_vacation_days, total_truancy_days)
...
def calculation_wages(working_hours_of_workers_sum_of_all_data: dict, secret_key_raw: int):
    """Функция для рассчета заработной платы. Функция принимает структуру данных, содержаших данные по рабочим часам и переработке, а так же 
    секретный ключ, введенный пользователем.  Если ключ корректный, создается структура, в которой содержиться оклад(зарплата без учета молока и премий), 
    ...
    for line in file_wage_rates:
        ...
        raw_data_rate_temporarily = raw_data_rate_encrypted[1] + \
            '.' + raw_data_rate_encrypted[0]
