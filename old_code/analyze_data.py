...
def translation_into_russian_names(eng_name: str, type_name: str):
    ...
    if type_name == 'month':
        months_names = {'January': 'Января', 'February': 'Февраля', 'March': 'Марта', 'April': 'Апреля', 'May': 'Мая', 'June': 'Июня',
        ...
        weekday_names = {'Monday': 'Понедельник', 'Tuesday': 'Вторник', 'Wednesday': 'Среда',
...
def definition_of_working_day(date: str):
    ...
    name_holiday_file = path.join(
        'data', 'variable_data_for_app', name_holiday_file_relative)
    name_postponed_working_days = path.join(
        'data', 'variable_data_for_app', name_postponed_working_days_relative)
    file_holiday = open(name_holiday_file, 'r', encoding='utf-8')
    file_postponed_working_days = open(
        name_postponed_working_days, 'r', encoding='utf-8')
    list_holiday = []
    ...
    for line in file_postponed_working_days:
        ...
        postponed_working_days = str(
            year) + '-' + month_postponed_working_days + '-' + day_postponed_working_days
        list_postponed_working_days.append(postponed_working_days)
    number_day = weekday(year, month, day)
    week_day_dic = {0: 'Понедельник', 1: 'Вторник', 2: 'Среда',
...
def search_for_missed_marks_employee(time_table: dict, id: int, requested_year: int, requested_month: int):
    ...
    last_day_month_type = datetime(
        requested_year, requested_month, last_day_month)
    ...
    if list_marks != 0 or list_missed_day != 0:
        ...
        if list_marks != 0:
            ...
            for marks in list_marks:
                ...
                month_name_rus_str = translation_into_russian_names(
                    month_name_eng_str, 'month')
                weekday_name_rus_str = translation_into_russian_names(
                    weekday_name_eng_str, 'weekday')
                output_str = '\t\t\t' + number_day_mark_str + ' ' + month_name_rus_str + \
                    ' | ' + time_mark_str + ' | ' + weekday_name_rus_str
                print(output_str)
                print('\t\t\t---------------------')
        ...
        if list_missed_day != 0:
            ...
            for missed_day in list_missed_day:
                ...
                weekday_name_rus_str = translation_into_russian_names(
                    weekday_name_eng_str, 'weekday')
                output_str = '\t\t\t' + number_day_mark_str + ' ' + \
                    month_name_rus_str + ' | ' + weekday_name_rus_str
                print(output_str)
                print('\t\t\t------------------')
...
def analyze_data_for_edit(time_table: dict, id: int, year: int, month: int):
    ...
    if list_marks != 0:
        ...
        for cell in list_marks:
            ...
            print('Дата и  время отметки, сохраненной в системе:',
                  time_table[date_key_for_edit][id][0], sep=' ')
            print(
    ...
    if list_missed_day != 0:
        print("ПРЕДУПРЕЖДЕНИЕ! " + name_employee +
              " не имеет данных за рабочий день!", file=stderr)
        for missed_day in list_missed_day:
            ...
            while loop_variable != 1:
                ...
                if switch_variable == '1':
                    ...
                    try:
                        time_table[missed_day][id] = [
                            data_to_write_begin, data_to_write_end]
