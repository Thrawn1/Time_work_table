...
def id_employee(name_id_file_relative='id_employee.dat', name_roles_file_relative='roles_employee.dat', type_data=1):
    ...
    name_id_file = path.join(
        'data', 'variable_data_for_app', name_id_file_relative)
    name_roles_file = path.join(
        'data', 'variable_data_for_app', name_roles_file_relative)
    if type_data == 1:
...
def verification_of_identity_for_permission_to_calculate(id:int):
    """Функция возвращает True если id работника предназначен для расчетов, иначе False"""
    file_name = path.join('data', 'variable_data_for_app', 'settlement_exceptions.dat')
    file = open(file_name, 'r', encoding='utf-8')
    exception_list_id = []
    for line in file:
        id_exeption = int(line.rstrip('/n'))
        exception_list_id.append(id_exeption)
    if id in exception_list_id:
        return False
    else:
        return True